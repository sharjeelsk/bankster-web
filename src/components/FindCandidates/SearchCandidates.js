...
function SearchCandidates(props) {
    ...
    const handleCandidateSearch = async (date, refine) => {
        let obj = {
            must: refine ? [] : must,
        ...
        await axios.post(`${process.env.REACT_APP_DEVELOPMENT}/api/recruiter/searchCandidate`, obj, { headers: { token: props.user.user } })
            .then(res => {
                console.log("================================================1234",isLoading)
                        ...
                        {candidates ? <>
                            ...
                            <div className="menu row m-auto align-items-center">
                                ...
                                <div className="ml-auto">
                                    <Button
                                        style={{ display: "flex", right: "229px" }}
                                        id="basic-button"
                                ...
                                <TablePagination
                                    ...
                                    count={candidates.length} // Total candidates count
                                    page={page}
                                    onPageChange={handleChangePage}
                                    rowsPerPage={rowsPerPage}
                                    onRowsPerPageChange={handleChangeRowsPerPage}
                                    rowsPerPageOptions={[50]}
                                    labelDisplayedRows={({ page, count }) => `Page ${page > 1 ? page + 1 : 1} of ${page > 1 && page > 100 ? Math.ceil(count / rowsPerPage) : page < 100 ? Math.ceil(count / rowsPerPage) : 100}`}
                                />
                            ...
                            <div>
                                {
                                    candidates && candidates.length > 0 ? (
                                        <>
                                            {/* Map over candidates and display them */}
                                            <div className="row m-auto">
                                                {candidates.slice((page - 1) * rowsPerPage, page * rowsPerPage).map((item, index) => (
                                                    <div className="col-12" key={index}>
                                                        <div className="row">
                                                            <div className="col-1 mt-4">
                                                                <Checkbox
                                                                    checked={selectedCandidate.filter(i => i._id === item._id).length > 0}
                                                                    onChange={(e) => {
                                                                        if (e.target.checked) {
